...
\begin{document}
  ...
  \small
  \gresetinitiallines{0}
  \gabcsnippet{(c4)<c><v>\Vbar</v>.</c> Di(h)vi(h)sé(h)runt(h) sí(h)bi(h) ves(h)ti(h)mén(i')ta(h) mé(g.)a(g.) (::) <c><v>\Rbar</v>.</c> Et(h) su(h)per(h) vés(h)tem(h) mé(h)am(h) mi(h)sé(i')runt(h) sór(g.)tem(g.) (::)}
  \bigskip
  \normalsize
  \begin{center}
